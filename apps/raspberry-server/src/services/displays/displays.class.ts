...
export class Displays extends Service<Display> {
  ...
  public async sync() {
    ...
    try {
      ...
      this.app.service("displays").emit("displays-sync-finish", { status: "failed" });
    ...
    this.app.service("displays").emit("displays-sync-finish", { status: "finish" });
  }
  ...
  public async connect(
    ...
    if (!connection) return { connected: false, reason: "No websocket connection" };
